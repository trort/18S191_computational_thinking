...
function matrix_to_vecvec(matrix)
	vec = [matrix[i,:] for i in 1:size(matrix, 1)]
	return vec
end
...
# ╔═╡ dfc03eb0-3e86-11eb-0c90-b140074065e9
[RGB(0.25, 0.31, 0.24), quantize(RGB(0.25, 0.31, 0.24))]
...
# ╔═╡ 1282c0e0-3e8a-11eb-2a78-4b2f2360d9c0
v[100]
...
function blur_1D(v, l)
	v_blured = copy(v)
	for i in 1:length(v)
		v_blured[i] = mean([extend(v, i+j) for j in -l:l])
	end
	return v_blured
end
...
@bind l_box Slider(0:1:8, show_value=true)

# ╔═╡ bdc9f3b0-3e8a-11eb-3218-d19cd3e9be77
colored_line(v)

# ╔═╡ d067bac0-3e8a-11eb-1fee-75dab5fd4ba7
...
# ╔═╡ 5abc4c60-3e8e-11eb-13ba-fb66eaf81007
...
@bind gaussian_kernel_size_1D Slider(0:1:8, show_value=true) # change this value, or turn me into a slider!

# ╔═╡ 7eb9ee30-3e8c-11eb-1375-3598680c08cd
