...
function intersection_distance(photon::Photon, wall::Wall)
	D = -dot(photon.p - wall.position, wall.normal) / dot(photon.l, wall.normal)
	return D
end
...
function intersection(photon::Photon, wall::Wall; ϵ=1e-3)
	...
	if D > ϵ
		p = photon.p + D * photon.l
		return Intersection(wall, D, p)
	else
		return Miss()
	end
end
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	...
	filter!(x -> x>ϵ, distances)
	
	if length(distances) == 0
		return Miss()
	else
		p = photon.p + first(distances) .* photon.l
		return Intersection(sphere, first(distances), p)
	end
end
...
function closest_hit(photon::Photon, objects::Vector{<:Object})
	hits = [intersection(photon, o) for o in objects]
	return minimum(hits)
end
...
# ╔═╡ 4ffe1b60-7006-11eb-1f79-3d32dc123276
test_sphere
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	return Photon(hit.point, refracted_normal, new_ior)
end
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	t = accumulate(1:N, init = photon) do p, _
		step_ray(p, scene)
	end
	return t
end
...
# ╔═╡ afe6a960-700c-11eb-0e46-41bf048642a6
@bind lecture_lens_ior Slider(0.5:0.1:5; default=1.5, show_value=true)

# ╔═╡ 270762e4-1ca4-11eb-2fb4-392e5c3b3e04
let
	lecture_lens = Sphere([0, 0], 5, lecture_lens_ior)
	lecture_scene = [
		Wall([14,0],[-1,0]),
		Wall([-10,0],[1,0]),
		Wall([0,-5],[0,1]),
	...
	scene = [lecture_lens, lecture_scene...]
	p = plot_scene(scene, legend=false, xlim=(-11,15), ylim=(-6,6))
	
	lecture_ray_N = 4
	
	lecture_photons = [Photon([-10,i], [1,0], 1.0) for i in -4:4]
	for photon in lecture_photons
		path = accumulate(1:lecture_ray_N; init=photon) do old_photon, _
			step_ray(old_photon, scene)
		end
