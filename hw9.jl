...
More negative B makes it less sensitive to CO2 change,
"""
...
let
	B_range = (-2:0.01:0)
	ECS_range = ECS.(B=B_range)
	plot(B_range, ECS_range, yaxis=:log, legend=:none,
		 xlabel="B", ylabel="ECS"
	    )
end
...
histogram(ECS_samples, size=(600, 250), label=nothing, xlabel="ECS [°C]", ylabel="samples")
...
# ╔═╡ 749d5e50-7a1e-11eb-247d-b1534a963324
sum(ECS_samples .> ECS(B=mean(B_samples))) / length(ECS_samples)
...
plot(t, simulated_rcp85_model.T, 
	 ylabel="Simulated temp [°C]")
...
let
	simulations = temperature_response.(Model.CO2_RCP85, B_samples)
	temp_2100 = last.(getproperty.(simulations, :T))
	prob_gt_2 = sum(temp_2100 .> 16) / length(temp_2100)
end

# ╔═╡ 8fbb2c50-7a21-11eb-060e-51afa7c490a4
let
	simulations = temperature_response.(Model.CO2_RCP85, B_samples)
	temp_2100 = last.(getproperty.(simulations, :T))
	histogram(temp_2100, size=(600, 250), label=nothing,
		xlabel="2100 Temp [°C]", ylabel="samples")
end
...
# ╔═╡ 9138e420-7a4c-11eb-1b16-a1d64a741087
begin
	struct History
		recent_CO2
		recent_T
	...
	function add_entry!(h::History, CO2::Number, T::Number)
		if length(h.recent_CO2) > h.max_size
			popfirst!(h.recent_CO2)
			popfirst!(h.recent_T)
		end
		
		push!(h.recent_CO2, CO2)
		push!(h.recent_T, T)
	end
...
@bind log_CO2 Slider(1:0.01:6; default=log10(280))

# ╔═╡ d713f4f0-7a4a-11eb-2e61-a9a8d1d0f12b
CO2 = 10^log_CO2
...
let
	...
	# your code here
	step_model!(ebm, CO2)
	cur_CO2 = ebm.CO2(ebm.t[end])
	cur_T = ebm.T[end]
	
	add_entry!(history, cur_CO2, cur_T)
	...
	plot!(p, history.recent_CO2, history.recent_T, 
		label=nothing,
		color=:black,
	)
...
# ╔═╡ 29e87f80-7a4f-11eb-349d-0fd1013a9629
