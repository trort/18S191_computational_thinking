...
κ controls how fast diffusion is.
u & v control the flow speed (advection strength).
N is just the resolution, increasing N increases resolution but also run time.
"""
...
With a multiplier of 2^9, the grid was showing alternating temp in adjacent cells and then all converge to NaN.
...
cloud_resolution_possible_at = let
	comp_power_ratio = (40000 / 700)^4
	possible_year = round(log2(comp_power_ratio) * 2 + 2020)
end
...
function absorbed_solar_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	S = S_at.(model.grid.y;grid=model.grid, S_mean=model.params.S_mean) * ones(size(model.grid.x))
	return S .* (1 .- α(T, model)) ./ (4 * model.params.C)
end

# ╔═╡ 020b75c0-8095-11eb-3918-cb616b241d67
let
	grid = Grid(10, 6000e3)
	params = RadiationOceanModelParameters()
	model = RadiationOceanModel(grid, params)
	
	#S = S_at.(model.grid.y;grid=model.grid, S_mean=model.params.S_mean) * ones(size(model.grid.x))
	S_at.(model.grid.y;grid=model.grid, S_mean=model.params.S_mean)
end
...
function runtime(N)
	runtime_grid = Grid(N, 6000.0e3);
	runtime_model = OceanModel(runtime_grid)
	runtime_T_init = InitBox(runtime_grid; value=50)
	
	Δt = 12*60*60
	runtime_sim = ClimateModelSimulation(runtime_model, runtime_T_init, Δt)
	
	return @elapsed for _ in 1:10
		timestep!(runtime_sim)
	end
end
...
begin
	rt_N = 10:10:100
	rt_runtime = runtime.(rt_N)
end

# ╔═╡ 1e0a3320-7fc2-11eb-20f6-c7d09efef63e
let
	p = plot(xlabel = "N", ylabel = "runtime [s]")
	plot!(p, rt_N, rt_runtime, label = "exp")
	
	pred_rt_runtime_coeff = rt_runtime[end] / (rt_N[end]^2)
	pred_rt_runtime = pred_rt_runtime_coeff .* rt_N.^2
	plot!(p, rt_N, pred_rt_runtime, label = "pred")
end |> as_svg
...
I found 2 stable states. With `T_init_value = -5` or lower. The entire ocean will freeze. With `T_init_value = -4` or higher. Only the north half of the ocean will freeze.
...
I found the entire ocean will freeze no matter how high `T_init_value` is.
...
I found the entire ocean will unfreeze no matter how low `T_init_value` is.
...
With αi = 0.8, and S = 2000. Bifurcation of entirely frozen and entirely unfrozen can be observed.
"""
...
The mean absorption is too low/high that the variance due to surface temperature is insufficient to unfreeze/freeze even the coldest/hottest grid.
