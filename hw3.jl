...
function clean(text)
	...
	text |> unaccent |> lowercase |> x->filter(isinalphabet, x)
end
...
unused_letters = let
	filter(x->sample_freqs[index_of_letter(x)]==0, alphabet)
end
...
double_letters = filter(x->sample_freq_matrix[index_of_letter(x), index_of_letter(x)]>0, alphabet)
...
row_col_answer = md"""
* sum of each column is $(sum(sample_freq_matrix,dims=1))
* sum of each row is $(sum(sample_freq_matrix,dims=2)')
Both arrays are just the frequency of each letter to appear.
"""
...
function completions_cache(grams)
	...
	for gram in grams
		k = gram[1:length(gram)-1]
		if haskey(cache, k)
			push!(cache[k], gram[end])
		else
			cache[k] = [gram[end]]
		end
	end
