...
function finite_difference_slope(f::Function, a, h=1e-5)
...
function euler_integrate_step(fprime::Function, fa::Number, 
	...
	return h * fprime(a + h) + fa # why fprime(a+h)
end
...
There is an outbreak. `I` goes to zero. Not everyone get infected but most.
"""
...
@bind β_interactive Slider(0.05:0.05:1, default=0.3, show_value=true)

# ╔═╡ d8743e60-6337-11eb-2073-d1df362de0dc
@bind γ_interactive Slider(0.05:0.05:1, default=0.1, show_value=true)

# ╔═╡ eaedb490-6337-11eb-2e05-77c28fab012b
plot_sir!(plot(), sir_T, 
	euler_SIR(β_interactive, γ_interactive, 
		[0.99, 0.01, 0.00], 
		sir_T))
...
function gradient_descent_1d_step(f, x0; η=0.01)
	gradient = finite_difference_slope(f, x0)
	return x0 - η * gradient
end
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	gradient_x, gradient_y = gradient(f, x0, y0)
	return x0 - η * gradient_x, y0 - η * gradient_y
end

# ╔═╡ 55153030-6344-11eb-1500-d1931dc049d5
(missing < 1E-6)

# ╔═╡ 8a114ca8-12e8-11eb-2de6-9149d1d3bc3d
begin
	function gradient_descent_2d(f, x0, y0;
			       η=0.01, max_steps=10000, min_change=1E-10)
		i = 0
		pos_change = min_change + 1
		x, y = x0, y0
		while i < max_steps && pos_change > min_change
			next_x, next_y = gradient_descent_2d_step(f, x, y; η=η)
	...
	function gradient_descent_2d_simple(f, x0, y0; η=0.01)
		max_steps = 10000
...
@bind N_gradient_2d Slider(0:40)
...
function loss_dice(μ, σ)
	gauss_y = gauss.(dice_x, μ, σ)
	loss = sum((gauss_y - dice_y).^2)
	return loss
end
...
found_μ, found_σ = let
	μ0 = 30
	σ0 = 10
	gradient_descent_2d(loss_dice, μ0, σ0,
		η = 0.5, max_steps=10000, min_change=1E-10)
end
...
# ╔═╡ 23b707e0-6360-11eb-2954-7b8930632168
[0.0187, 0.0021]
...
# ╔═╡ f940a730-6366-11eb-182e-3fb146fc16e2
...
function loss_sir(β, γ)
	sir_0 = [0.99, 0.01, 0.00]
	sir_fit = euler_SIR(β, γ, sir_0, hw4_T)
	err = sir_fit - hw4_results
	loss = mean([mean(err_t.^2) for err_t in err])
	return loss
end
