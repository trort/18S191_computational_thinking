...
ex_1_1 = md"""
if `sqrt(x)` is larger than both `x/a` and `a`, then the product `sqrt(x) * sqrt(x) = x` will be larger than `x/a * a = x`. Similarly for the case of `sqrt(x)` less than both `x/a` and `a`.
""" 
...
function newton_sqrt(x, error_margin=0.001, a=x / 2) # a=x/2 is the default value of `a`
