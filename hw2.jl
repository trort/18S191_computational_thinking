...
function remove_in_each_row(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		j = clamp(j, 1, n)
		img′[i, :] = vcat(img[i, 1:j-1], (j==n) ? [] : img[i, j+1:end] )
	end
	img′
end
...
# ╔═╡ 7f020c2e-460a-11eb-18f7-2f891d7ea37c
[[1 2 3]; [4 5 6]]
...
function remove_in_each_row_no_vcat(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		...
		# to avoid using `vcat`.
		#img′[i, :] = vcat((@view img[i, 1:j-1]), (@view img[i, j+1:end]))
		#img′[i, :] .= [@view img[i, 1:j-1]; @view img[i, j+1:end]]
		j = clamp(j, 1, n)
...
~ 1/3 allocations are reduced because there is no need to create an array for the vcat version.
"""
...
Another ~2/3 is reduced with `@view` by avoiding copying the slices.
"""
...
# ╔═╡ 37422e70-460f-11eb-32bc-81a33c6dce21
[max(convolve(brightness.(img), Kernel.sobel()[1])...),
	min(convolve(brightness.(img), Kernel.sobel()[1])...)]
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:size(energies, 1)
		next_ids = clamp.(collect(-1:1) .+ seam[i-1], 1, size(energies, 2))
		min_idx = argmin([energies[i,j] for j in next_ids])
		seam[i] = next_ids[min_idx]
	end
	return seam
end

# ╔═╡ 758c7620-4611-11eb-0b34-f77e966c9e36
clamp.(collect(-1:1) .+ 3, 1, 3)
...
# ╔═╡ 9b950650-4613-11eb-28b3-1b8d7355eff3
random_seam(size(greedy_test)..., 1)
...
function least_energy(energies, i, j)
	...
	if i == size(energies, 1)
		...
		candidates = clamp.(collect(-1:1) .+ j, 1, size(energies, 2))
		lower_energies = [least_energy(energies, i+1, k) for k in candidates]
		return energies[i, j] + min(lower_energies...)
	end
end
...
function recursive_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 2:m
		lower_idx = clamp.(collect(-1:1) .+ seam[i-1], 1, n)
		lower_energies = [least_energy(energies, i, k) for k in lower_idx]
		seam[i] = lower_idx[argmin(lower_energies)]
	end
	return seam
end
...
* Each pixel at the higher level will visit all 3 the pixels of lower level, which are all possible routes from this pixel. And all top level pixels will be visited. So all possible routes from all starting points will be visited.
* It grows O(n * 3^m)
"""
...
function memoized_least_energy(energies, i, j, memory)
	...
	if !haskey(memory, (i, j))
		if i == m
			...
			candidates = clamp.(collect(-1:1) .+ j, 1, n)
			lower_energies = [memoized_least_energy(energies, i+1, k, memory)
				              for k in candidates]
			memory[(i,j)] = energies[i, j] + min(lower_energies...)
		end
	end
	return memory[(i,j)]
end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	for i in 2:m
		lower_idx = clamp.(collect(-1:1) .+ seam[i-1], 1, n)
		lower_energies = [memoized_least_energy(energies, i, k, memory)
			              for k in lower_idx]
		seam[i] = lower_idx[argmin(lower_energies)]
	end
	return seam
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if memory[i,j] <= 0
		if i == m
			...
			candidates = clamp.(collect(-1:1) .+ j, 1, n)
			lower_energies = [matrix_memoized_least_energy(energies, i+1, k, memory)
				              for k in candidates]
			memory[i,j] = energies[i, j] + min(lower_energies...)
		end
	end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 2:m
		lower_idx = clamp.(collect(-1:1) .+ seam[i-1], 1, n)
		lower_energies = [matrix_memoized_least_energy(energies, i, k, memory)
			              for k in lower_idx]
		seam[i] = lower_idx[argmin(lower_energies)]
	end
	return seam
...
function least_energy_matrix(energies)
	m, n = size(energies)
	least_energy = zeros(size(energies))
	for i in m:-1:1, j in 1:n
		if i == m
			least_energy[i,j] = energies[i,j]
		else
			candidates = clamp.(collect(-1:1) .+ j, 1, n)
			lower_least_energies = [least_energy[i+1, k] for k in candidates]
			least_energy[i, j] = energies[i, j] + min(lower_least_energies...)
		end
	end
	return least_energy
end

# ╔═╡ ae67a4e0-4717-11eb-027d-57422a8281b9
float_to_color.(least_energy_matrix(energy(img)) ./5)
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i in 2:m
		lower_idx = clamp.(collect(-1:1) .+ seam[i-1], 1, n)
		lower_energies = [least_energies[i,k] for k in lower_idx]
		seam[i] = lower_idx[argmin(lower_energies)]
	end
	return seam
end
...
if shrink_matrix
	matrix_carved = shrink_n(img, 100, matrix_memoized_seam)
...
# ╔═╡ 28ef0760-4625-11eb-3805-478f3b1edd1d
float_to_color.(energy(pika))
