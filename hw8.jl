...
begin
	...
	function interact(ray::Photon, hit::Intersection{Sphere},
			          num_intersections::Integer, objects::Vector{<: Object})
		hit_normal = sphere_normal_at(hit.point, hit.object)
		going_out = (ray.l) ⋅ (hit_normal) > 0
		hit_surface = hit.object.s
		
		reflected_normal = reflect(ray.l, hit_normal)
		reflected_ray = Photon(hit.point, reflected_normal, ray.c, ray.ior)
		if hit_surface.r > 0 #&& !going_out
			reflected_ray = step_ray(reflected_ray, objects, num_intersections - 1)
		end
		
		old_ior, new_ior = 1.0, hit.object.s.ior
		if going_out
			old_ior, new_ior = new_ior, old_ior
		end
		refracted_normal = refract(ray.l, hit_normal, old_ior, new_ior)
		refracted_ray = Photon(hit.point, refracted_normal, ray.c, new_ior)
		if hit_surface.t > 0
			refracted_ray = step_ray(refracted_ray, objects, num_intersections - 1)
		end
		
		new_color = (hit_surface.r * reflected_ray.c 
			+ hit_surface.t * refracted_ray.c 
			+ hit_surface.c.alpha * RGB(hit_surface.c)
		)
		return Photon(ray.p, ray.l, new_color, ray.ior)
	end
...
main_scene = [
	...
	Sphere([-50,0,-25], 20, 
		Surface(0.1, 0.0, RGBA(0, .3, .8, 0.9), 1.0)),
	
	Sphere([50, 0, -25], 20,
		Surface(0.75, 0.0, RGBA(.1,.9,.1,0.25), 1.5)),
	...
	Sphere([0,50,-100], 20, 
		Surface(0.0, 1.0, RGBA(0,0,0,0.0), 1.5)),
	
	Sphere([125, 125, -200], 100, 
		Surface(0.5, 0.5, RGBA(0,0,0,0.0), 1.1)),
	
	Sphere([-35, -35, -50], 10, 
		Surface(0, 0.9, RGBA(1,1,1,0.1), 5)),
]

# ╔═╡ 1f66ba6e-1ef8-11eb-10ba-4594f7c5ff19
let
	cam = Camera((800,600), 16, -12, [0,10,100])

	ray_trace(main_scene, cam; num_intersections=25)
end
...
escher_cam = Camera((400,400), 30, -10, [0,00,30])
...
# ╔═╡ ca44cf52-748d-11eb-1c72-a32731cd1aa6
RGB((rand() * ones(3))...)
...
function padded(img)
	m = size(img)[1]
	n = size(img)[2]
	pad_img = similar(img, 3 * m, 3 * n)
	noisify = pixel -> RGB((rand() * ones(3))...)
	pad_img = noisify.(pad_img)
	pad_img[m+1:2m, n+1:2n] = img
	return pad_img
end
