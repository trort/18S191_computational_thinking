...
begin
	Pkg.add(["Plots", "PlutoUI", "Statistics"])
...
Everyone recover on day 1, giving $\tau = 1$
"""
...
# ╔═╡ 48badb40-593b-11eb-1798-bdaeda6363a9
function mean(data)
...
# ╔═╡ 6e7cded0-5938-11eb-2c68-bbd27ea53467
@bind N_interactive Slider(1:1:100_000, show_value=true)

# ╔═╡ 715a3530-5938-11eb-2719-c3738ed6fa91
...
function frequencies_plot_with_expected(p, N)
	# start out by copying the frequencies_plot_with_maximum function
	experiment = do_experiment(p, N)
	base = frequencies_plot_with_mean(experiment)
	
	T = 1:1:maximum(experiment)
	plot!(base, T, p * (1-p).^(T.-1), label="expected", markershape = :circle)
	
	return base
end

# ╔═╡ 88e50350-5939-11eb-1a7e-c74a67a6c907
frequencies_plot_with_expected(p_interactive, N_interactive)
...
# ╔═╡ c3d518f0-593a-11eb-0808-65861140f693
let
	N = 10_000
	p_array = 0.01:0.01:1
	recovery_day_array = [mean(do_experiment(p, N)) for p in p_array]
	plot(p_array, recovery_day_array)
end
...
# ╔═╡ e49d1020-59c5-11eb-31f6-db5172316403
Integer(R)
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	plot!(p, 1:T, avg_S, lw=3, label="Susceptable")
	plot!(p, 1:T, avg_I, lw=3, label="Infected")
	plot!(p, 1:T, avg_R, lw=3, label="Recovered")
...
@bind p_infection Slider(0.01:0.01:0.2, show_value=true)

# ╔═╡ 1a8218f0-59d2-11eb-269c-6fa8c14ebdac
@bind p_recovery Slider(0.01:0.01:0.2, show_value=true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	avg_I = mean(getproperty.(simulations, :I))
	avg_R = mean(getproperty.(simulations, :R))
	std_S = std(getproperty.(simulations, :S))
	std_I = std(getproperty.(simulations, :I))
	std_R = std(getproperty.(simulations, :R))
	
	plot!(p, 1:T, avg_S, yerr=std_S, 
		markerstrokecolor=:auto, markerstrokealpha=0.2, markerstrokewidth=0.1,
		lw=3, label="Susceptable")
	plot!(p, 1:T, avg_I, yerr=std_I, 
		markerstrokecolor=:auto, markerstrokealpha=0.2, markerstrokewidth=0.1, 
		lw=3, label="Infected")
	plot!(p, 1:T, avg_R, yerr=std_R, 
		markerstrokecolor=:auto, markerstrokealpha=0.2, markerstrokewidth=0.1, 
		lw=3, label="Recovered")
...
# ╔═╡ 710eea30-59ce-11eb-1ce7-ff7cd14f1c73
...
let
	...
	avg_I = mean(getproperty.(simulations, :I))
	plot!(p, 1:T, avg_I, alpha=1, lw=3, label="mean")
...
# ╔═╡ 66572210-59d3-11eb-183f-9199d37fd899
sir_mean_error_plot(simulations)

# ╔═╡ 6590e050-59dd-11eb-080e-83f0a0fcc507
let
	sim = simulations[4]
	(max(sim.I...), 100 - last(sim.S), findfirst(sim.I .== 0) )
end

# ╔═╡ 2db960e0-59d2-11eb-04d7-ad4149a01be9
let
	infection_interactive = InfectionRecovery(p_infection, p_recovery)
	simulations_interactive = repeat_simulations(100, 1000, infection_interactive, 20)
	sir_mean_plot(simulations_interactive)
end

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
let
	simulations_large = repeat_simulations(100, 1000, InfectionRecovery(0.02, 0.002), 2_000)
	bar(frequencies(last.(getproperty.(simulations_large, :I))))
end
...
simulations_reinfection = repeat_simulations(100, 1000, Reinfection(0.02, 0.002), 20)

# ╔═╡ c3eadbe0-59df-11eb-2414-57176fffa355
let
	p = plot()
	T = length(first(simulations).S)
	...
	avg_I = mean(getproperty.(simulations_reinfection, :I))
	plot!(p, 1:T, avg_I, alpha=1, lw=3, label="mean")
...
md"""
The behaviour is qualitatively different as I converge to a positive value instead of 0.
"""
